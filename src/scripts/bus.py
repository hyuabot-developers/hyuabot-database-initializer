...
async def insert_bus_route_stop(db_session: Session):
    bus_route_stop_list = [
        dict(
            route_id="216000026", stop_id="216000719", stop_seq=12,
            start_stop_id=217000292, minute_from_start=25,
        ),  # 3100(한양대정문)
        dict(
            route_id="216000096", stop_id="216000719", stop_seq=12,
            start_stop_id=217000292, minute_from_start=25,
        ),  # 3100N(한양대정문)
        dict(
            route_id="216000043", stop_id="216000719", stop_seq=12,
            start_stop_id=217000292, minute_from_start=25,
        ),  # 3101(한양대정문)
        dict(
        ...
        dict(
            route_id="216000061", stop_id="216000381", stop_seq=18,
            start_stop_id=233003145, minute_from_start=28,
        ),  # 3102(한국생산기술연구원)
        dict(
            route_id="216000061", stop_id="216000379", stop_seq=19,
            start_stop_id=233003145, minute_from_start=29,
        ),  # 3102(ERICA컨벤션센터)
        dict(
            route_id="216000068", stop_id="216000383", stop_seq=12,
            start_stop_id=216000358, minute_from_start=18,
        ),  # 10-1(한양대기숙사앞)
        dict(
            route_id="216000068", stop_id="216000381", stop_seq=13,
            start_stop_id=216000358, minute_from_start=19,
        ),  # 10-1(한국생산기술연구원)
        dict(
            route_id="216000068", stop_id="216000379", stop_seq=14,
            start_stop_id=216000358, minute_from_start=20,
        ),  # 10-1(ERICA컨벤션센터)
        dict(
            route_id="216000068", stop_id="216000138", stop_seq=21,
            start_stop_id=216000138, minute_from_start=0,
        ),  # 10-1(상록수역3번출구)
        dict(
        ...
        dict(
            route_id="200000015", stop_id="216000070", stop_seq=44,
            start_stop_id=217000626, minute_from_start=31,
        ),  # 9090(한양대입구)
        dict(
            route_id="216000075", stop_id="216000759", stop_seq=15,
            start_stop_id=216000358, minute_from_start=22,
        ),  # 50(안산파크푸르지오)
        dict(
            route_id="216000075", stop_id="216000117", stop_seq=79,
            start_stop_id=213000487, minute_from_start=59,
        ),  # 50(성포주공4단지)
    ]
    ...
    insert_statement = insert_statement.on_conflict_do_update(
        ...
        set_=dict(
            ...
            start_stop_id=insert_statement.excluded.start_stop_id,
            minute_from_start=insert_statement.excluded.minute_from_start,
        ),
    )
    db_session.execute(insert_statement)
    db_session.commit()
